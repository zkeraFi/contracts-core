...
contract ZlpManager is ReentrancyGuard, Governable, IZlpManager {
    ...
    constructor(address _vault, address _usdg, address _zlp, address _shortsTracker, uint256 _cooldownDuration, IBlockInfo _blockInfo) public {
        gov = msg.sender;
        vault = IVault(_vault);
        usdg = _usdg;
        zlp = _zlp;
    ...
    function _removeLiquidity(address _account, address _tokenOut, uint256 _zlpAmount, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        require(lastAddedAt[_account].add(cooldownDuration) <= blockInfo.getBlockTimestamp(), "ZlpManager: cooldown duration not yet passed");
