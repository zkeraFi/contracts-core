...
const { deployContract, deployTimeDistributor, deployRewardDistributor, deployVault, deployVaultPriceFeed, deployZkeTimelock } = require("../shared/fixtures")
...
describe("ZkeTimelock", function () {
  ...
  beforeEach(async () => {
    ...
    timelock = await deployZkeTimelock([
      wallet.address,
      5 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
  ...
  it("inits", async () => {
    ...
    await expect(deployZkeTimelock([
      wallet.address,
      7 * 24 * 60 * 60 + 1,
      7 * 24 + 60 * 60,
      rewardManager.address,
  ...
  it("setBuffer", async () => {
    const timelock0 = await deployZkeTimelock([
      user1.address,
      3 * 24 * 60 * 60,
      7 * 24 * 60 * 60,
      rewardManager.address,
