...
contract MigrateVesting is Ownable_0_8_0 {
    ...
    function totalZkePurchased() public view returns(uint256) {
        return _totalZkePurchased + vesting.totalZkePurchased();
    }
    ...
    function participants(address account) public view returns (uint256,uint256,uint256) {
        ...
        (uint256 zkePurchased,,) = vesting.participants(account);
        zkePurchased = participant.zkePurchased + zkePurchased;

        return (zkePurchased, participant.zkeClaimed, participant.lastClaimed);
    }
